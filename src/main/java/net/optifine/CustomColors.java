...
public class CustomColors {
	...
	private static final CustomColors.IColorizer COLORIZER_GRASS = new CustomColors.IColorizer() {
		public int getColor(IBlockState blockState, IBlockAccess blockAccess, BlockPos blockPos) {
			...
			return CustomColors.swampGrassColors != null && biome == Biomes.SWAMPLAND
					...
					: biome.getGrassColorAtPos(blockPos);
		}
	...
	private static final CustomColors.IColorizer COLORIZER_FOLIAGE = new CustomColors.IColorizer() {
		public int getColor(IBlockState blockState, IBlockAccess blockAccess, BlockPos blockPos) {
			...
			return CustomColors.swampFoliageColors != null && biome == Biomes.SWAMPLAND
					...
					: biome.getFoliageColorAtPos(blockPos);
		}
		...
		for (int i = 0; i < astring.length; ++i) {
			...
			try {
				...
				if (inputstream == null) {
					...
					properties.load(inputstream);
					inputstream.close();
					CustomColormap customcolormap = new CustomColormap(properties, s, width, height,
							paletteFormatDefault);
	...
	private static CustomColormap[][] blockListToArray(List list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            List<CustomColormap[]> subList = (List)list.get(i);
		...
		if (blockColormaps != null) {
			...
			if (customcolormap != null) {
				if (Config.isSmoothBiomes() && !customcolormap.isColorConstant()) {
					return getSmoothColorMultiplier(blockState, blockAccess, blockPos, customcolormap,
	...
	public static int getXpOrbColor(float timer) {
		if (xpOrbColors == null) {
			...
			int i = (int) Math
					.round((double) ((MathHelper.sin(timer) + 1.0F) * (float) (xpOrbColors.getLength() - 1)) / 2.0D);
	...
	public static void updateWaterFX(Particle fx, IBlockAccess blockAccess, double x, double y, double z,
			RenderEnv renderEnv) {
	...
	private static int getLilypadColorMultiplier(IBlockAccess blockAccess, BlockPos blockPos) {
		return lilyPadColor < 0
				? getBlockColors().colorMultiplier(Blocks.WATERLILY.getDefaultState(), blockAccess, blockPos, 0)
				: lilyPadColor;
	}
	...
	public static Vec3d getUnderFluidColor(IBlockAccess blockAccess, double x, double y, double z,
	...
	public static boolean updateLightmap(World world, float torchFlickerX, int[] lmColors, boolean nightvision,
		...
		if (world == null) {
			...
			if (j >= 0 && j < lightMapPacks.length) {
				...
				return lightmappack == null ? false
	...
	public static Vec3d getWorldFogColor(Vec3d fogVec, World world, Entity renderViewEntity, float partialTicks) {
		...
		if (dimensiontype == DimensionType.NETHER) {
			...
			return getFogColor(fogVec, minecraft.world, renderViewEntity.posX, renderViewEntity.posY + 1.0D,
	...
	public static Vec3d getWorldSkyColor(Vec3d skyVec, World world, Entity renderViewEntity, float partialTicks) {
		...
		if (dimensiontype == DimensionType.OVERWORLD) {
			return getSkyColor(skyVec, minecraft.world, renderViewEntity.posX, renderViewEntity.posY + 1.0D,
					renderViewEntity.posZ);
		} else {
	...
	public static int getColorFromItemStack(ItemStack itemStack, int layer, int color) {
		if (itemStack == null) {
			...
			if (item == null) {
				...
				return item instanceof ItemMonsterPlacer
