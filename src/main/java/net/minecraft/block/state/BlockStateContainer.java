...
public class BlockStateContainer
{
    ...
    protected BlockStateContainer(Block p_i2_1_, IProperty<?>[] p_i2_2_, ImmutableMap < IUnlistedProperty<?>, Optional<? >> p_i2_3_)
    {
        this.block = p_i2_1_;
        Map < String, IProperty<? >> map = Maps. < String, IProperty<? >> newHashMap();

        for (IProperty<?> iproperty : p_i2_2_)
        {
        ...
        for (List < Comparable<? >> list1 : Cartesian.cartesianProduct(this.getAllowedValues()))
        {
            ...
            BlockStateContainer.StateImplementation blockstatecontainer$stateimplementation = this.createState(p_i2_1_, ImmutableMap.copyOf(map1), p_i2_3_);
